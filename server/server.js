...
// Определяем порт в зависимости от среды
const PORT = process.env.NODE_ENV === 'production' 
  ? (process.env.PORT || 3001) 
  : (process.env.PORT || 3000);

// Настройки CORS для разных сред
const corsOptions = {
  origin: process.env.NODE_ENV === 'production' 
    ? ['file://', 'app://*'] // Для Electron production
    : ['http://localhost:5173', 'http://localhost:5174'], // Для development
  credentials: true,
...
// 🩺 ПРОВЕРКА ЗДОРОВЬЯ СЕРВЕРА (обновлено для Electron)
app.get('/api/health', (req, res) => {
  res.json({ 
    ...
    electron: process.env.NODE_ENV === 'production'
  });
});
...
server.listen(PORT, 'localhost', async () => {
  ...
  console.log(`📡 CORS: ${environment === 'production' ? 'Electron app' : 'http://localhost:5173'}`);
